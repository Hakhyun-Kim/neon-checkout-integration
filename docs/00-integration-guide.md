...
| 3 | Wrong currency, wrong payment methods, wrong tax | Country derived from a language signal | Resolve country server-side: explicit choice → geo header, and only from a proxy you trust to set it and strip the client's copy → default. Never the UI toggle, and never `Accept-Language` — a browser language is not a location. Let a weak signal *suggest* a market the player accepts with a click; resolve the real one from IP |
