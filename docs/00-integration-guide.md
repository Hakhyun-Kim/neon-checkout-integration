...
| 3 | Wrong currency, wrong payment methods, wrong tax | Country derived from UI language | Resolve country server-side: explicit choice → geo header, and only from a proxy you trust to set it → `Accept-Language` region → default. Never the language toggle. Treat the language step as a stopgap: a browser language is not a location, so production should resolve it from IP |
