...
These are the things I would raise in a first working session with a Korean studio
integrating Neon. They are the reason the integration is shaped the way it is.
...
The list a studio will ask about, roughly in order of importance:
...
checkout. This single answer usually decides whether a Korean studio adopts.
...
banner: it stays entirely outside that regime. A studio selling gacha through a
...
handles the withdrawal notice and refund obligation, or whether the studio must
...
- **PIPA / 개인정보보호법** — if payment data is processed outside Korea, the
  ...
  discloses on the hosted page and what the studio must add in-game.
- **전자금융거래법** — the merchant-of-record model is what makes Neon workable
  for a foreign studio selling into Korea; it is worth stating plainly to a
  partner, because it is usually their first legal question.
