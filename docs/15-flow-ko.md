...
**샌드박스에서 발견한 벤더 이슈 (2026-09-07 재검증):** 실패하는 것은 "빈 본문"이
아니라 **전체 환불 경로 자체**입니다. `{}`도, 문서가 안내하는 `{"fee": 0}`도 모두
`500 UNKNOWN_ERROR`를 돌려줍니다. 반면 잘못된 본문(`{"fee": "x"}`)은 깔끔한 `400`을,
본문 없음은 `415`를 돌려주므로 **검증 단계는 정상이고 핸들러 안에서 터진다**는 뜻입니다.
같은 구매에 대한 item 단위 부분 환불은 `201`로 성공합니다. 또 구매 응답은 필드명이
`items[].id`인데 환불 요청은 `itemId`를 받습니다. 전체 실험 표와 재현 절차는
[09](09-sandbox-run.md)에 있습니다.
...
- 동시 요청 두 건이 각각 `201`로 만들어지고 공급자 호출도 두 번 일어나는 것까지
  재현했습니다. 보유 검사는 entitlement가 생긴 뒤에만 막기 때문입니다. 실제 이중
  청구까지 시험하지는 않았고(Neon이 이전 세션을 무효화할 수도 있습니다), 원자적
  예약과 만료·정합 처리는 미구현입니다. → [12](12-review.md)
