...
청구 국가는 ①명시적 선택 ②신뢰하는 프록시가 붙인 지리 헤더(`TRUST_GEO_HEADERS`)
③기본값 KR 순서로 **서버가** 정합니다. 어떤 언어 신호도 이 목록에 없습니다 —
게임의 ko/en 토글도, 브라우저 `Accept-Language`도. 서울의 영어 브라우저 사용자는
미국 거주자가 아니기 때문입니다.

대신 약한 신호는 **제안**은 할 수 있습니다. 브라우저 지역은 카탈로그 응답의
`suggestion`으로 내려와 스토어 맨 위에 한 줄로 뜹니다 — *"브라우저가 US 같아요.
결제를 USD로 바꿀까요?"* 플레이어가 누르면 그 순간 ①번 명시적 선택이 되고 제안은
사라집니다. **추론은 무엇을 권할지를 정하고, 무엇을 신고할지는 사람이 정합니다.** 가격표에
없는 나라를 KR로 바꿔 신고하지도 않습니다 — 두 마켓 외의 국가는 **Neon Global
Store**가 USD로 처리하므로, 일본 플레이어는 `JP`로 신고되고 USD로 결제합니다.
실제 위치는 IP에서 와야 하며, 그건 코드가 아니라 배포 변경입니다 —
`deploy/README.md`에 지오 프록시를 앞에 두고 직접 경로를 닫는 절차가 있습니다.
