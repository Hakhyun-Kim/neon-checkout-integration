...
③브라우저 `Accept-Language` 지역 ④기본값 KR 순서로 **서버가** 정합니다. 게임의
ko/en 토글은 이 목록에 없습니다. ③은 데모용 타협입니다 — 브라우저 언어는 위치가
아니어서 서울의 영어 브라우저 사용자가 미국으로 청구되고, CDN이 없는 이 배포에서는
사실상 유일하게 작동하는 추론 신호입니다. 운영에서는 IP 지오로케이션으로 대체해야
합니다.
